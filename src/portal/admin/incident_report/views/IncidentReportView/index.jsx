...
import { GET_INCIDENT_REPORT, INCIDENT_REPORT_SOLVED } from "../../api";
...
const IncidentReportView = () => {
  ...
  const [tagAsCompleted, tagAsCompletedOpts] = usePost({
    onComplete: () => {
      notify("success", "Incident Report has been updated!");
      refetch();
      navigate("/portal/admin/incident_report");
    },
    onError: () => {
      refetch();
    },
  ...
  return (
    <Card>
      ...
      {incident_report_data?.status === "PENDING" ? (
        <Space>
          <Button
            type="primary"
            color="red"
            onClick={() =>
              tagAsCompleted({
                url: INCIDENT_REPORT_SOLVED,
                data: {
                  id,
                },
              })
            }
            loading={tagAsCompletedOpts.loading}
          >
            <strong>Tag as Solved</strong>
          </Button>
        </Space>
      ) : null}
    </Card>
